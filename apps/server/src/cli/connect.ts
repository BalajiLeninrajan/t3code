...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ...
    : !status.desired
      ...
      : !status.linked
        ? "Start B5 Code to provision the environment link and launch its managed tunnel."
        : undefined;
    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (linked) {
          ...
          yield* Console.log(
            flags.publishOnly
              ? `✓ Authorized${connectedAs(linked.identity)}\n\nNext\n  Start B5 Code to publish agent activity (no managed tunnel).`
    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (yield* CliState.readCliDesiredCloudLink) {
          yield* Console.log(
            "A T3 Connect link is already pending. Start B5 Code to finish provisioning it; publishing starts once it links.",
          );
          return;
        }
        yield* CliState.setCliDesiredCloudLink(true, "publish_only");
        yield* Console.log(
          "Restart B5 Code to finish authorizing this environment to publish (no managed tunnel is created).",
        );
      }),
    ),
  ),
  ...
  Command.withHandler((flags) =>
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (background) {
          yield* Console.log(
            "\n✓ Background service ready\n\nB5 Code will stay reachable after you log out.",
          );
          return;
        }
        const serveCommand = yield* resolveCliCommand("serve");
