...
> For maintainers. Using B5 Code? See [docs/user](../user/).
...
1. Creates three temporary T3 base directories and starts a local server for each on an available
   port.
2. Creates B5 Code, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic B5 Code review diff.
...
The Pending rows use the production offline outbox and point at the real B5 Code and React fixture
