# Running B5 Code in the Background

On a Linux host, B5 Code can run as a background service for your user. It starts when the machine
boots and keeps running after you log out.

## Manage the Service

Install it with the latest B5 Code release:
...
Updating restarts B5 Code briefly. Let active agent work and terminal commands finish first.

The systemd unit runs a small stable launcher. Exact B5 Code versions are installed separately, so
...
want B5 Code to start in the background.
