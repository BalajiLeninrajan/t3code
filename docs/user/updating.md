# Keeping B5 Code in Sync

The B5 Code web or desktop app and the server it connects to work best when they use the same
version. If they do not match, B5 Code shows a warning with the right update option for that server.
...
| Action                     | What to do                                                                                                                                                                  |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Update server**          | Available for the B5 Code Linux background service. Select the button and leave B5 Code open while it prepares, tests, restarts, and reconnects.                            |
| **Update the desktop app** | Open the B5 Code desktop app on the machine that runs the server and install the app update there. Reopen it if needed.                                                     |
| **Copy update command**    | Copy the command, open a terminal on the server machine, stop the current B5 Code server, and relaunch it with the copied command and any startup options you normally use. |

The available action depends on how that server was started. B5 Code does not update connected
...
If the server instead runs as the B5 Code background service, update the service on the host and
...
See [Running B5 Code in the Background](./background-service.md) for install, status, and removal
commands.
